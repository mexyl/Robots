...
int main()
{
	...
	{
		walk_acc(&rbt, i, totalCount, iniEE, d, h, alpha, beta, "-y", "z", pIn_acc[i], pEE_acc[i], pBodyEp_acc[i]);
	}
	...
	for (unsigned i = 0; i < totalCount; ++i)
	{
		walk_dec(&rbt, i, totalCount, iniEE, d, h, alpha, beta, "-y", "z", pIn_dec[i], pEE_dec[i], pBodyEp_dec[i]);
	}
	...
	{
		walk_const(&rbt, i, totalCount, iniEE, d, h, alpha, beta, "-y", "z", pIn_const[i], pEE_const[i], pBodyEp_const[i]);
	}
