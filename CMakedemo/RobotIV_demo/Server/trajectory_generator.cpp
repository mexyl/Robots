...
int home2start(unsigned count , Aris::RT_CONTROL::CMachineData &data)
{
	const unsigned period1 = 1000;
	const unsigned period2 = 3000;
...
int execute_cmd(unsigned count,const ROBOT_CMD &cmd,Aris::RT_CONTROL::CMachineData &data)
{
	...
	case HOME2START_2:
		...
		break;
	case MV_FORWARD:
		if(count<cmd.param[0].toUInt)
		{
			ret=walk_acc(count,data,cmd);
		}
		else if( count < (2*cmd.param[9].toUInt-1)*cmd.param[0].toUInt )
		{
			ret=walk_const((count+cmd.param[0].toUInt)%(2*cmd.param[0].toUInt),data,cmd);
		}
		else
		{
			ret=walk_dec(count%cmd.param[0].toUInt , data , cmd);
		}
